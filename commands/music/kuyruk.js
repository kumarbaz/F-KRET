...
module.exports = class ViewQueueCommand extends Command {
	...
	run(msg, { page }) {
		...
		if (!queue) return msg.reply('Burda Bir Şarkı Çalmıyor. Neden Birtane Açmıyorsunuz?');
		...
		return msg.embed({
			...
			description: stripIndents`
				__**Şarkı Kuyruğu, Sayfa ${paginated.page}**__
				...
				**Şimdi Oynayan:** ${!isNaN(currentSong.id) ? `${currentSong.name}` : `[${currentSong.name}](${`https://www.youtube.com/watch?v=${currentSong.id}`})`}
				${oneLine`
					**Zaman:**
					${!currentSong.playing ? 'Durduruldu: ' : ''}${Song.timeString(currentTime)} /
					${currentSong.lengthString}
					(${currentSong.timeLeft(currentTime)} left)
				`}
				**Kuyruğun Toplam Zamanı:** ${Song.timeString(totalLength)}
			`
